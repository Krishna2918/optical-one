 ...
 * timestamptz stays a Date from the driver (Better Auth needs that on the raw
 * PGLite client). App SQL goes through sanitizeRow so JSON stays safe.
...
function createNeonSql(): Promise<Sql> {
  globalRef.__pgSqlPromise__ ??= (async () => {
    ...
    return toSql(async <T>(text: string, params: unknown[]) => {
      ...
      return (res.rows as T[]).map(sanitizeRow);
    });
  })().catch((err) => {
...
async function createPgliteSql(): Promise<Sql> {
  ...
  globalRef.__pgliteInstance__ ??= (async () => {
    ...
    const { loadPgliteBootOptions } = await import("./pglite-boot.server");
    const boot = await loadPgliteBootOptions();
    const pg = new PGlite({
      parsers: {
        [OID_INT8]: Number,
        [OID_DATE]: identity,
