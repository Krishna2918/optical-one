...
const walk = (dir) => {
  for (const ent of readdirSync(dir, { withFileTypes: true })) {
    ...
    if (ent.name === "_libs") dests.push(p);
    else walk(p);
  }
};
walk(functionsRoot);
...
if (dests.length === 0) {
  throw new Error(
    "[pglite-assets] no _libs dirs under .vercel/output/functions — PGLite would 500 on Vercel",
  );
}
...
console.log(`[pglite-assets] copied ${copied} files into ${dests.length} _libs dir(s)`);
